...
```{r}
size <- nrow(ds)
ranges <- c(0, 1, 2, 3, 4, 5, 8, 10, 15, 25, 50, 75, 100, 200, 500, 1000, 3000, 10000)
...
ggplot(temp1, aes(fill=Type, y=value, x=as.factor(temp1$max))) + geom_bar(position = "dodge", stat = "identity", alpha = density_alpha) + theme_light(base_size = 9) + xlab("Size, LOC, $ecdf(x)$") + ylab("Ratio") + labs(fill = "Type") + scale_fill_brewer(palette="Paired") + scale_y_continuous(breaks = seq(0, 1, 0.1), limits = c(0, 1)) + theme(axis.text.x = element_text(angle = 90, vjust = .4)) + scale_x_discrete(labels = function(n){format(as.numeric(n), digits = 0, big.mark = ",")})
